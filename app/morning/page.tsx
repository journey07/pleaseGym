...
const readCoachSource = async () => {
  try {
    ...
    if (response.ok) {
      ...
      if (data.state) {
        const history = Array.isArray(data.state.history) ? data.state.history as StoredSession[] : [];
        const favorites = Array.isArray(data.state.favorites) ? data.state.favorites as StoredFavorite[] : [];
        window.localStorage.setItem("first-rep-history", JSON.stringify(history));
        window.localStorage.setItem("first-rep-favorites", JSON.stringify(favorites));
...
const buildCoachContext = async () => {
  ...
  const history = source.history
    .filter((session) => typeof session.date === "string" && Array.isArray(session.exercises))
    .sort((a, b) => String(b.date).localeCompare(String(a.date)))
    .slice(0, 6)
    .map((session) => ({
      date: session.date,
      exercises: (session.exercises ?? []).slice(0, 8).map((exercise) => {
        const sets = Array.isArray(exercise.sets) ? exercise.sets.filter((set) => set.done) : [];
        const isDistance = exercise.metric === "distance";
        const heaviest = sets.reduce((best, set) => set.weight > best.weight ? set : best, { weight: 0, reps: 0 });
        return {
          ...
          distanceKm: isDistance ? sets.reduce((sum, set) => sum + (set.distanceKm ?? 0), 0) : 0,
        };
      }),
    }));
...
const storeDecision = (decision: Decision) => {
  ...
  const current = readArray<{ date: string; decision: Decision; decidedAt: string; xp?: number }>(key)
    .filter((item) => item.date !== todayKey());
  current.push({ date: todayKey(), decision, decidedAt: new Date().toISOString(), xp: decision === "go" ? 100 : 0 });
...
const calculateMissionStats = (): MissionStats => {
  const decisions = readArray<{ date?: string; decision?: Decision; xp?: number }>("first-rep-morning-decisions");
  const activeDates = new Set(decisions
  ...
  readArray<StoredSession>("first-rep-history").forEach((session) => {
    if (typeof session.date === "string") activeDates.add(session.date.slice(0, 10));
  });
  ...
  const weekGoes = Array.from({ length: 7 }, (_, index) => shiftDateKey(today, -index))
    .filter((key) => activeDates.has(key)).length;
  const totalXp = decisions.reduce((sum, item) => sum + (item.decision === "go" ? (item.xp ?? 100) : 0), 0);
...
export default function MorningBridge() {
  ...
  const [missionStats, setMissionStats] = useState<MissionStats>({ streak: 0, weekGoes: 0, totalXp: 0 });
  ...
  useEffect(() => {
    setDateLabel(new Intl.DateTimeFormat("ko-KR", {
      month: "long",
      day: "numeric",
      weekday: "long",
    }).format(new Date()));
  ...
  const choose = async (nextDecision: Decision) => {
    ...
    try {
      ...
      if (!response.ok || !data.coach) {
        const suffix = data.code === "openai_not_configured"
          ? " 서버에 OPENAI_API_KEY를 설정하면 바로 활성화됩니다."
          : "";
        throw new Error(`${data.error ?? "AI 코치 응답을 받지 못했습니다."}${suffix}`);
      }
      setCoach(data.coach);
      setStatus("done");
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : "AI 코치 연결에 실패했습니다.");
      setStatus("error");
    }
  };
  ...
  const startSkipHold = () => {
    ...
    holdInterval.current = window.setInterval(() => {
      setSkipProgress(Math.min(100, ((performance.now() - startedAt) / 3000) * 100));
    }, 40);
  ...
  const usePlan = () => {
    ...
    window.localStorage.setItem("first-rep-coach-draft", JSON.stringify({
      date: todayKey(),
    ...
    <main className="morning-page">
      <section className="morning-card">
        <header className="morning-head">
          <Link href="/" aria-label="운동 달력으로 돌아가기"><span>1</span><b>FIRST REP</b></Link>
          <p>{dateLabel || "오늘"}</p>
        </header>
        ...
        <div className="mission-hud" aria-label="운동 퀘스트 현황">
          <div>
            ...
            <p><b>{missionStats.streak || "NEW"}</b><small>DAY STREAK</small></p>
          </div>
          <div className="mission-progress">
            <span><b>WEEK MISSION</b><small>{missionStats.weekGoes}/4 출석</small></span>
            <i><em style={{ width: `${Math.min(100, (missionStats.weekGoes / 4) * 100)}%` }} /></i>
          </div>
          <div className="xp-total"><small>TOTAL XP</small><b>{missionStats.totalXp.toLocaleString("ko-KR")}</b></div>
        </div>

        <div className="morning-copy">
          <span>06:00 · TODAY&apos;S QUEST</span>
          <h1>{decision === null ? "첫 세트를 쟁취하라." : decision === "go" ? "퀘스트를 수락했다." : "오늘 퀘스트를 포기했다."}</h1>
          <p>{decision === null
            ? "목표는 운동을 잘하는 게 아닙니다. 헬스장에 도착해 첫 세트를 시작하는 것입니다."
            : "결정은 기록됐습니다. AI가 바로 다음 행동만 정리합니다."}</p>
        </div>
        ...
        {decision === null && !skipOpen && (
          <section className="quest-card" aria-label="오늘의 운동 퀘스트">
            ...
            <div className="quest-objective">
              ...
              <div><small>OBJECTIVE</small><strong>헬스장에 가서 첫 세트 완료</strong></div>
            </div>
            <button className="quest-accept" onClick={() => choose("go")} disabled={status === "loading"}>
              <span>퀘스트 수락 · 지금 출발</span><b aria-hidden="true">→</b>
            </button>
            <button className="skip-open" onClick={() => setSkipOpen(true)}>오늘은 패스</button>
          </section>
        )}
        ...
        {decision === null && skipOpen && (
          <section className="skip-gate" aria-label="오늘 운동 포기 확인">
            ...
            <h2>정말 오늘을 넘길 건가요?</h2>
            <p>회복이 필요한 날이라면 괜찮습니다. 다만 순간의 귀찮음이라면, 다시 퀘스트로 돌아가세요.</p>
            <button
              className={`hold-to-skip ${skipHolding ? "holding" : ""}`}
              style={{ "--hold-progress": `${skipProgress}%` } as React.CSSProperties}
              onPointerDown={(event) => {
            ...
            >
              <span>{skipHolding ? "계속 누르세요" : "3초 길게 눌러 포기 확정"}</span>
              <i aria-hidden="true" />
            </button>
            <button className="return-quest" onClick={() => {
              clearSkipHold();
              setSkipOpen(false);
            }}>← 다시 퀘스트로 돌아가기</button>
          </section>
        ...
        {decision !== null && (
          <div className={`decision-lock ${decision}`}>
            <span>{decision === "go" ? "QUEST ACCEPTED" : "QUEST ABANDONED"}</span>
        ...
        {status === "error" && (
          <section className="coach-error" role="alert">
            ...
            <p>{error}</p>
            <div>
              <button onClick={() => decision && choose(decision)}>다시 연결</button>
          ...
          <section className="coach-result">
              ...
              <div className="coach-plan">
                ...
                {coach.exercises.map((exercise, index) => (
                  <div key={`${exercise.name}-${index}`}>
                    ...
                    <small>{exercise.metric === "distance"
                      ? `${exercise.targetValue}km`
                      : `${exercise.sets} × ${exercise.reps} · ${exercise.targetValue || "직접 입력"}kg`}</small>
                  </div>
                ))}
              </div>
            )}
            ...
            <div className="coach-actions">
              {coach.exercises.length > 0
                ...
                : <Link className="primary" href="/">오늘 결정 완료</Link>}
