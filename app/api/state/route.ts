...
export async function GET() {
  ...
  try {
    const [state] = await getDb().select().from(userState).where(eq(userState.ownerId, ownerId())).limit(1);
    ...
    return NextResponse.json({ configured: true, error: "Neon에서 기록을 불러오지 못했습니다." }, { status: 502 });
  }
}
...
export async function PUT(request: Request) {
  ...
  if (!Array.isArray(body.history) || !Array.isArray(body.favorites)) {
    return NextResponse.json({ error: "운동 기록과 즐겨찾기는 배열이어야 합니다." }, { status: 400 });
  }

  const coachMemory = Array.isArray(body.coachMemory) ? body.coachMemory : [];
  const now = new Date();

  try {
    await getDb().insert(userState).values({
      ownerId: ownerId(),
    ...
    return NextResponse.json({ error: "Neon에 기록을 저장하지 못했습니다." }, { status: 502 });
  }
}
